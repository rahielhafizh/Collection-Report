...
from services.cleaner.clean_summary_recovery_wo import clean_path
...
from services.mailer.outlook_summary_recovery_wo import send_outlook_email
...
def excel_config():
    ...
    os.startfile(CONFIG["WORKSOURCE_RECOVERY_WO"])
    ...
    # ── SAVE NEW WORKBOOK ─────────────────────────────────────────────────────
    save_new_book()
    pyautogui.write(CONFIG["SUBMISSION_RECOVERY_WO"])
    confirm()
    wait_timer(CONFIG["WAIT_TIME"]["FIVE_SECOND"])
...
if __name__ == "__main__":
    ...
    stop_keeper()
    wait_timer(CONFIG["WAIT_TIME"]["ONE_SECOND"])
    clean_path(target_folder=CONFIG["SUBMISSION_RECOVERY_WO"])
    wait_timer(CONFIG["WAIT_TIME"]["ONE_SECOND"])
