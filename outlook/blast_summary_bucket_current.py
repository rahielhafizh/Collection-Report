...
from services.cleaner.clean_summary_bucket_current import clean_path
...
from services.mailer.outlook_summary_bucket import send_outlook_email
...
def excel_config():
    ...
    # ── OPEN & NAVIGATE ───────────────────────────────────────────────────────
    os.startfile(CONFIG["WORKSOURCE_BUCKET_CURRENT"])
    ...
    save_new_book()
    pyautogui.write(CONFIG["SUBMISSION_BUCKET_CURRENT"])
    confirm()
...
if __name__ == "__main__":
    ...
    stop_keeper()
    wait_timer(CONFIG["WAIT_TIME"]["ONE_SECOND"])
    clean_path(target_folder=CONFIG["SUBMISSION_BUCKET_CURRENT"])
    wait_timer(CONFIG["WAIT_TIME"]["ONE_SECOND"])
