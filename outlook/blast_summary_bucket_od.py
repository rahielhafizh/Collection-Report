...
from services.cleaner.clean_summary_bucket_od import clean_path
...
from services.mailer.outlook_summary_bucket_od import send_outlook_email
...
def excel_config():
    ...
    bucket_filename = (
        f"Summary Performance Bucket OD {bucket_day} {month_idn_title}"
    )
...
def send_email():
    ...
    subject_email = f"Summary Performance Bucket OD {bucket_day} {month_idn_title} {bucket_year}"
...
if __name__ == "__main__":
    ...
    stop_keeper()
    wait_timer(CONFIG["WAIT_TIME"]["ONE_SECOND"])
    clean_path(target_folder=CONFIG["SUBMISSION_BUCKET_OD"])
    wait_timer(CONFIG["WAIT_TIME"]["ONE_SECOND"])
