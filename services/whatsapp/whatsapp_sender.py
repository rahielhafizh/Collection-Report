...
import webbrowser
from services.check.chrome_checker import open_chrome
...
def send_paste_report(phone_no: str, message: str) -> bool:
    try:
        open_chrome()
        normalised = formatting_phone_number(phone_no)
        logger.info(f"[WHATSAPP] INITIATING PASTE MESSAGE TO {normalised}")
        ...
        wait_timer(CONFIG["WAIT_TIME"]["THREE_SECOND"])
        closing_web_tab()
        logger.info(f"[WHATSAPP] PASTE MESSAGE SENT TO {normalised}")
        return True
