...
class ScreenKeeperState:
    ...
    def get_adaptive_sleep(self, ase: float) -> float:
        try:
            cpu = psutil.cpu_percent(interval=0.05)
            return max(0.005, 0.05 * (1.0 - (cpu / 200)))
        except Exception:
            return False
...
def main() -> None:
    ...
    if args.stop:
        sys.exit(0 if stop_keeper() else 1)
